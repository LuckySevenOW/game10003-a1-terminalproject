...
//This particular intro is taken from Warhammer 40,000's first edition, and is basically the most succinct way to explain the setting to anyone unfamiliar.
//I could have done it in my own words, but it'd be fairly obvious it was just re-written in my own words, and would probably be less concise. I hope this isn't an issue.
...
//Getting the player's name. I phrased this hoping they'll use some sort of 40k-ish name, as it fits better with the text, but it doesn't really matter at the end of the day.
//Also just using console.readline to get the input.
Console.WriteLine("What do your battle brothers call you?");
Console.WriteLine("(Please enter your name, Sergeant.)\n");
string name = Console.ReadLine();

// Here's where I get how many years their character has served, and this will change the dialogue to either refer to them as a regular sergeant, or a veteran sergeant.
// Also using parsing to parse string into integer.
Console.WriteLine("\nHow many years have you served, Sergeant?");
Console.WriteLine("Note: Astartes may serve for hundreds of years, and Veterans may have served for 150 years or more.\n");

int yearsServed = int.Parse(Console.ReadLine());
bool veterancy = false;

// Here's where I bring in the boolean, set to false if they are not a veteran, set to true if they are. 
...
//Using string interpolation to fill in your character's name, and using the boolean to change which dialogue is shown.

if (veterancy == true)
{
    Console.WriteLine($"\nYou are Sergeant {name} of the Dark Angels chapter of the Adeptus Astartes, leading the 6th Squad of the 5th Company, serving under Master Lazarus. You are an experienced Veteran of the Chapter, with countless battles and honors to your name, and are a fearsome opponent for anyone that should oppose the Emperor's will.\n");
    Console.WriteLine("You and your squad of seasoned and experienced Veteran Marines have been sent by direct order of Master Lazarus to head into a Necron Tomb World, and assassinate the Necron Overlord in charge before he is able to awaken his armies. You have been given explicit orders to avoid awakening the Necron tomb. If the tomb is awakened, the Dark Angels may be forced to order an exterminatus on the tomb world, essentially nuking the entire planet into oblivion before the Necrons can awaken, and start causing problems for the star system. This would result in the deaths of all of the Imperial Citizens on the planet, and would also destroy all of the precious resources the planet holds. Resources that the Imperium has great need for.\n");
}

else
{
    Console.WriteLine($"\nYou are Sergeant {name} of the Dark Angels chapter of the Adeptus Astartes, leading the 6th Squad of the 5th Company, serving under Master Lazarus. You are a skilled and reliable battle brother, and a fearsome opponent for anyone that should oppose the Emperor's will.\n");
    Console.WriteLine("You and your squad of Space Marines have been sent by direct order of Master Lazarus to head into a Necron Tomb World, and assassinate the Necron Overlord in charge before he is able to awaken his armies. You have been given explicit orders to avoid awakening the Necron tomb. If the tomb is awakened, the Dark Angels may be forced to order an exterminatus on the tomb world, essentially nuking the entire planet into oblivion before the Necrons can awaken, and start causing problems for the star system. This would result in the deaths of all of the Imperial Citizens on the planet, and would also destroy all of the precious resources the planet holds. Resources that the Imperium has great need for.\n");
}
...
if (approach == "Flank")
{
    ...
    //Now they'll have to pick between engaging or avoiding an Annihilation Barge, a Necron Tank.
    //As far as I'm concerned, it's a fool's errand for infantry to take on a tank alone without anti-tank weaponry, so there's no way you'll survive that,
    //but I at least wanted to give some kind of consolation prize in destroying the tank for those who may have seen taking it out as a noble move.
    if (tacticsFlank == "Power Swords")
    {
        ...
        //Finally, here's the confrontation with the Overlord. Going straight for him is obvious, but he's got a command console next to him, and I didn't include that as an option for no reason.
        //He's just going to summon reinforcments if you go for him, so it makes sense to me to go for the command console to stop his reinforcements.
        if (barge == "Avoid")
        {
            ...
            Console.WriteLine("\nMoving into the chamber, you spot the foul xenos leader, looking significantly more advanced than the infantry you have fought along the way. He is standing next to some sort of important-looking console, tapping buttons on its interface, when he notices your squad's entry.");
            ...
            if (overlordFlank == "Command Console")
            {
                ...
                Console.WriteLine("\nThe Overlord charges you, swinging his Hyperphase Glaive at you, the searing blade narrowly missing your visor. You twist and parry his attack away with your Power Sword. The few troops he has present also charge your squad, including several Warriors, and two Lychguard; Two members of Xerathas' personal Honor Guard. Your battle brothers are contending with his troops, so you'll have to deal with the Overlord yourself.");
                Console.WriteLine("\nYou and Xerathas dance back and forth, locked in a dance of life and death. He is a powerful foe, and while Astartes are among the Imperium's best, his skill has you wondering if his millenia of experience may be too much even for you and your skills. You continue like this for a while, battling back and forth, each hoping to find an opening, until you eventually spot one.");
                Console.WriteLine("\nOn the wall near the Overlord is some type of xenos interface panel. You only recognize it as such by its basic similarities to the command console you destroyed. Rapidly hatching a plan, you dash forward towards the Overlord, telegraphing that you are going to pass by him, only to throw your Power Sword right into the panel. Xerathas turns to follow your movement, but is immediately caught and stunned momentarily by the energy exploding from the panel.");
                Console.WriteLine("\nFortunately, all you need is a moment. In that moment, you grab the Overlord from behind, put him in headlock of sorts, and proceed to violently tear his head off of his body. The last things you hear of Xerathas are his cries of anguish and defeat, and moments later, the loud metal clank of his robotic body dropping to the tomb floor, lifeless.");
                Console.WriteLine("\nPerhaps if the Necrons were less arrogant, they would have seen through your plan and wouldn't have underestimated you like this.");
                Console.WriteLine("\nAfter the long and intense battle with one of the Necrons' best, through the power of the God-Emperor of Mankind, you prevailed. You have defeated Xerathas, Lord of Storms, and accomplished your primary objective.");
            ...
            else if (overlordFlank == "Assault")
            {
                ...
                Console.WriteLine("\n\"I am not pleased. Supreme Grand Master Azrael will have to hear about this, and we may have to order an exterminatus before things get out of hand. We will discuss possible penance later on. You are dismissed.\"");
                Console.WriteLine("\n\nMission Failed.");
            }
        }
    ...
    else if (tacticsFlank == "Bolt Rifles")
    {
        ...
        if (veterancy == true)
        {
            Console.WriteLine("\nWith all of the commotion, and all of the Necrons that are attracted to your position, it's becoming clear to you that the tomb is just beginning to awaken. The more Necrons you take out, the more keep coming, and soon enough, it's starting to become unmanageable, even for honored Veteran Astartes.");
        }

        else
        {
            Console.WriteLine("\nWith all of the commotion, and all of the Necrons that are attracted to your position, it's becoming clear to you that the tomb is just beginning to awaken. The more Necrons you take out, the more keep coming, and soon enough, it's starting to become unmanageable, even for the battle-hardened Astartes you are.");
        }
        ...
        Console.WriteLine("\nHeading out as fast as you can, you and your squad carve a path back to the surface, fighting tirelessly the entire way. Eventually, you manage to make it all the way back to the surface, albeit bloodied and bruised from all of the intense combat.");
...
else if (approach == "Head On")
{
    ...
    //Here, they'll have to pick between attacking or avoiding a Deathmark. In my opinion, when you're on a mission to hunt an Overlord, getting caught up on one elite sniper unit (that can easily kill your squad) is a bad idea.
    if (tacticsHeadOn == "Power Swords")
    {
        ...
        //This confrontation with the Overlord is the same. I would have just found a way to have it loop back to the same encounter if you made the right choices, so that I wouldn't need duplicates in the code here, but I didn't know how to do that, and this does the job just fine.
        if (deathmark == "Avoid")
        {
        ...
        else if (deathmark == "Attack")
        {
            ...
            Console.WriteLine("\nUp above the planet, in the Dark Angels Battle Barge, Master Lazarus speaks to Zakariah, his Lieutenant; \"Lieutenant, Squad 6 has been lost. We still need to deal with the Overlord before we can send in the PDF and the Guard. I need you to head in with two squads to finish the job before the Overlord begins awakening the rest of the tomb world.\" Zakariah replies, \"Very well. We will avenge our fallen brothers, and destroy the xenos Overlord.\"");
            Console.WriteLine("\n\nMission Failed.");
        }
    }
